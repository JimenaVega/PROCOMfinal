...
def arrayFixedInt(intWidth, fractWidth, N, signedMode='S', roundMode='trunc', saturateMode='saturate'):
  ...
  if(isinstance(N, (list, numpy.ndarray))):
    ...
    raise TypeError( "type(N) = {} not supported".format(type(N))) 
...
class DeFixedInt(object):
  ...
  def __init__(self, intWidth=0, fractWidth=15, signedMode='S', roundMode='trunc', saturateMode='saturate'):
    ...
    if( (roundMode != 'trunc') and
        ...
        (roundMode != 'round')):
      raise ValueError ("Round mode {} not supported!".format(roundMode))

    if( (saturateMode != 'saturate') and 
        (saturateMode != 'wrap')):
      raise ValueError ("Saturate mode {} not supported!".format (saturateMode))
        
    if( (signedMode != 'S') and 
        (signedMode != 'U')):
      raise ValueError ("Signed mode {} not supported!".format (signedMode))
  ...
  def _overflowCheck(self):
    ...
    ######################SE AGREGO EL WRAP####################        
          ...
          if(self.value > maxNum or self.value < minNum):
            msg = "Value: {} exeeds allowed range {} ... {}".format(self.value, minNum, maxNum)
...
#      
  ...
  def round(self, value):
    ...
    if(self.__roundMode == 'trunc'):
      ...
      raise "ERROR: DeFixedInt.round(): {} not supported round mode!".format(self.__roundMode)
  ...
  def showRange(self):
    ...
    if (self.signedMode=='S'):
        ...
        print ("S({}, {}): " .format(self.intWidth+self.fractWidth, self.fractWidth)),
        print ("{} ... {}" .format (min, max))
    elif(self.signedMode=='U'):
        min = 0
        max = 2**(self.intWidth)-2**(-self.fractWidth)
        print ("U({}, {}): ".format(self.intWidth+self.fractWidth, self.fractWidth)),
        print ("{} ... {}".format (min, max))
